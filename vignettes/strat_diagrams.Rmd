...
```{r, eval=FALSE}
alta_lake_geochem %>%
  mutate(param = fct_relevel(param, c("Ti", "Cu", "C/N"))) %>%
...
ggplot(combined_data, aes(x = value, y = depth)) +
  ...
  geom_point() +
  scale_y_reverse() +
  facet_geochem_gridh(vars(param), vars(location), scales = "free") +
  labs(x = NULL, y = "Depth (cm)")
```
